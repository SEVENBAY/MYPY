...
def getWebimage(weburl, image_pattern="jpg"):
    ...
    # 设置匹配图片地址的表达式
    reimg = r'http://\S*?\.%s' % image_pattern
    img_re = re.compile(reimg)

    # 读取指定的web页面内容
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.106"
                      " Safari/537.36"}
        ...
        for img in img_list:
            ...
            try:
                request.urlretrieve(img,
                                    r"C:\Users\Administrator\Desktop\img\{0}-{1}.{2}".format(now, index, image_pattern))
                success = True
            except Exception:
                ...
                if not success:
                    ...
                    with open(r"C:\Users\Administrator\Desktop\img\{0}-{1}.{2}".format(now, (index - 1), image_pattern),
                              "wb") as f:
                        f.write(res.content)
                    print("下载成功...")
                    fail_count -= 1
...
if __name__ == "__main__":
    getWebimage("http://sucai.redocn.com/font/jpgtupian/")
